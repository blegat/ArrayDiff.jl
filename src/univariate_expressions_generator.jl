...
_to_expr(f) = f === nothing ? :(nothing) :
              f isa Number  ? :($f * one(x)) :
              f

open("src/univariate_expressions.jl", "w") do io
    print(io, """
#! format:off
#
...
""")
    ...
    println(io, "value_deriv_and_second(::typeof(+), x) = (+x, one(x), zero(x))")
    println(io, "value_deriv_and_second(::typeof(-), x) = (-x, -one(x), zero(x))")
    println(io, "value_deriv_and_second(::typeof(abs), x) = (abs(x), ifelse(x >= 0, one(x), -one(x)), zero(x))")
    println(io, "value_deriv_and_second(::typeof(sign), x) = (sign(x), zero(x), zero(x))")

    for (op, _) in Calculus.symbolic_derivatives_1arg()
        f   = Expr(:call, op, :x)
        df  = _differentiate(f)
        d2f = _differentiate(df)
        println(
            io,
            "value_deriv_and_second(::typeof($op), x) = (",
            _to_expr(f), ", ",
            _to_expr(df), ", ",
            _to_expr(d2f),
            ")",
